...
LOG="/home/admin/clawd/data/claude-code-results/hook.log"
RESULT_DIR="/home/admin/clawd/data/claude-code-results"
META_FILE="${RESULT_DIR}/task-meta.json"
OPENCLAW_BIN="/home/admin/.npm-global/bin/openclaw"
DISCORD_USER_ID="1482202051256651816"  # 你的 Discord 用户 ID
DISCORD_ACCOUNT="pineapple"  # Discord bot 账号标识
...
${FILE_TREE}"
    ...
    "$OPENCLAW_BIN" message send \
        ...
        --message "$MSG" 2>/dev/null && log "Sent Discord DM to $DISCORD_USER_ID" || log "Discord DM failed"
...
📝 *摘要:* ${SUMMARY}"
        ...
        "$OPENCLAW_BIN" message send \
            ...
            --message "$CALLBACK_MSG" 2>/dev/null && log "Sent Discord callback to $CALLBACK_GROUP" || log "Discord callback failed"
    fi
...
📝 *摘要:* ${SUMMARY}"
        ...
        DM_CMD=("$OPENCLAW_BIN" message send --channel discord --account "$DISCORD_ACCOUNT" --target "$CALLBACK_DM" --message "$CALLBACK_MSG")
        [ -n "$CALLBACK_ACCOUNT" ] && DM_CMD+=(--account "$CALLBACK_ACCOUNT")

        "${DM_CMD[@]}" 2>/dev/null && log "Sent Discord DM callback to $CALLBACK_DM" || log "Discord DM callback failed"
    fi
fi
